...
int main( int argc, char *argv[] ) {
  ...
  long long tareaEnvenenada = 0;
   ...
   if(miId == 0){
	...
	MPI_Send(&tareaEnvenenada, 1, MPI_LONG_LONG_INT, 1, 99 , MPI_COMM_WORLD);
	MPI_Recv(&numPrimosPar, 1, MPI_INT, 1, 88 , MPI_COMM_WORLD, &s);
	
	}else if(miId == 1){
        MPI_Recv(&dato, 1, MPI_LONG_LONG_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &s);
  ...
  //
  // Implementacion Paralela bajo demanda.
  // =====================================
  // 
  ...
  if(miId == 0) {
    ...
    for(i = 1; i < numProcs; i++) {
      MPI_Recv(&peticion, 1, MPI_CHAR, MPI_ANY_SOURCE, 88, MPI_COMM_WORLD, &s);
      MPI_Send(&tareaEnvenenada, 1, MPI_LONG_LONG_INT, s.MPI_SOURCE, 99, MPI_COMM_WORLD);
    }
  }else{
  ...
  MPI_Reduce(&numPrimosPar, &suma, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
	
  MPI_Barrier( MPI_COMM_WORLD );
  t2 = MPI_Wtime(); ttPar = t2 - t1;
  if( miId == 0 ) {
    printf( "Implementacion paralela bajo demanda.    Tiempo (s): %lf\n", ttPar );
    printf( "Numero de primos en el vector: %d\n", suma );
  }
...
void construyeVector( int * numElementos, long long ** vectorNumeros ) {
  ...
  vecNums[  0 ] = 4LL;  vecNums[  1 ] = 15L;  vecNums[  2 ] = 2LL;  
  vecNums[  3 ] = 3LL;  vecNums[  4 ] = 4LL;  vecNums[  5 ] = 27LL; 
