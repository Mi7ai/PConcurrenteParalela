...
int main( int argc, char *argv[] ) {
  ...
  // ... (D)
  int ant = (miId == 0)?MPI_PROC_NULL:miId-1;
  int sig = (miId == numProcs - 1)?MPI_PROC_NULL:miId+1;
  
  sumaFinal = 0;
  MPI_Recv(&sumaFinal, 1, MPI_DOUBLE, sig, 88, MPI_COMM_WORLD, &s);
  sumaFinal += sumaLocal;
  MPI_Send(&sumaFinal, 1, MPI_DOUBLE, ant, 88, MPI_COMM_WORLD);
